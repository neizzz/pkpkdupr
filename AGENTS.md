...
- 운영 proxy는 하나의 SWAG이며, 도메인별 템플릿 원본은 `infra/swag/site-confs/pkpkdupr.conf.template`와 `pkelo-*.conf.template`입니다.
...
- 주 SWAG는 DuckDNS DNS-01을, `pkelo-certificate`는 Cloudflare DNS-01 인증서를 갱신합니다. 실제 서버 설정은 `/opt/pkpkdupr/data/certs/nginx/site-confs/pkpkdupr.conf`, `pkelo.conf`, `pkelo-ssl.conf`이며 배포 스크립트가 생성합니다.
